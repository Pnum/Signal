Noise
	Generator
		Grad
			get2(x, y)
				if(SIGNAL_USE_DLL)
					return text2num(call(SIGNAL_DLL_PATH, __DLL_GRAD2)(num2text(x), num2text(y), num2text(seed)))
			...
			get3(x, y, z)
				if(SIGNAL_USE_DLL)
					return text2num(call(SIGNAL_DLL_PATH, __DLL_GRAD3)(num2text(x), num2text(y), num2text(z), num2text(seed)))
